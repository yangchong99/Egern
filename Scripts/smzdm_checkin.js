 ...
 *
 * 使用：
 * - 请先用抓 Cookie 脚本将 Cookie 保存到本地存储，键名 smzdm_cookie
 * - 定时任务执行本脚本
 *
 * author: yangchongUncle (ported for Egern)
 * update-time: 2026-01-09 (port: 2026-02-11)
 */
...
function notify(title, sub, msg) {
  if (typeof $notify === "function") return $notify(title, sub, msg);
  if (typeof $notification !== "undefined" && typeof $notification.post === "function") {
    return $notification.post(title, sub, msg);
  }
}
...
function httpRequest(method, params) {
  // 统一成 Promise：resolve({ status, headers, body, raw })
  return new Promise((resolve, reject) => {
    const req = Object.assign({}, params, { method });

    // QX/Egern 常见：$task.fetch
    ...
    // Surge/Loon/Egern 常见：$httpClient
    if (typeof $httpClient !== "undefined") {
      ...
      if (method === "GET" && typeof $httpClient.get === "function") return $httpClient.get(req, cb);
      if (method === "POST" && typeof $httpClient.post === "function") return $httpClient.post(req, cb);
    }

    reject(new Error("No HTTP API ($task.fetch / $httpClient) available"));
  });
}
...
async function getUserNickName(cookie) {
  const ts = Date.now();
  const url = `https://zhiyou.smzdm.com/user/info/jsonp_get_current?with_avatar_ornament=1&callback=jQuery_${ts}&_=${ts}`;
  ...
  try {
    ...
    let body = (resp.body || "").toString();

    body = body.replace(/\/\*[\s\S]*?\*\//g, "").replace(/\/\/.*/g, "").trim();

    const jsonpMatch =
      /[a-zA-Z_$][\w$]*\s*\(\s*(\{[\s\S]*\})\s*\)\s*;?\s*$/.exec(body);
    ...
    log("请求用户信息异常：" + (e && e.message ? e.message : e));
    return null;
  }
}
...
async function androidSignIn(cookie, username) {
  if (!cookie || !username) return [false, "Cookie或用户名为空"];
  ...
  try {
    ...
    const resp = await httpPost({
      ...
      body: `sk=${encodeURIComponent(username)}&sign=${encodeURIComponent(
        sign
      )}&weixin=1&v=9.9.12&captcha=&f=android&token=${encodeURIComponent(
        smzdmToken
      )}&touchstone_event=&time=${encodeURIComponent(outcome)}000`,
    });
    ...
    try {
      ...
      return [false, `签到返回非JSON: ${text.substring(0, 200)}`];
    }

    if (data && data.error_code === "0") {
      const gold = (data.data && data.data.cgold) != null ? data.data.cgold : "0";
      const dailyNum = (data.data && data.data.daily_num) != null ? data.data.daily_num : "0";
      const cards = (data.data && data.data.cards) != null ? data.data.cards : "0";

      if (data.error_msg && String(data.error_msg).includes("签到成功")) {
        return [true, `签到成功\n本次获得金币+${gold}｜连续${dailyNum}天｜补签卡${cards}张`];
      } else if (data.error_msg === "已签到") {
        return [true, `今日已签到\n连续${dailyNum}天｜金币${gold}｜补签卡${cards}张`];
      }
    }

    return [false, `签到失败: ${text.substring(0, 250)}`];
  } catch (e) {
    return [false, `签到异常: ${e && e.message ? e.message : e}`];
  }
}
...
(async () => {
  try {
    ...
    if (!cookie) {
      notify("什么值得买", "签到失败", "未找到 Cookie（smzdm_cookie），请先抓取/更新");
      log("没有找到 Cookie，退出");
      return done({});
    }
